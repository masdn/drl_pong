...
class QNetwork(nn.Module):
    """
    Q-network that approximates Q(s,a) for all actions.
...
class SARSAAgent:
    ...
    On-policy TD control method that learns Q(s,a) using:
    ...
    def select_action(self, state):
        ...
        if self.use_boltzmann:
            ...
            # Shift Q-values to avoid numerical issues
    ...
    def update(self, state, action, reward, next_state, next_action, done):
        ...
        # Get next Q-value: Q(s',a')
        with torch.no_grad():
            q_next = self.q_network(next_state_tensor)[next_action]
        
        # Compute target: r + gamma * Q(s',a') (or just r if done)
        if done:
    ...
    def train(self, num_episodes, max_steps):
        ...
        for episode in range(num_episodes):
            ...
            for step in range(max_steps):
                # Take action in environment (R, S')
                ...
                # SARSA update using (S, A, R, S', A')
                loss = self.update(state, action, reward, next_state, next_action, done or truncated)
...
if __name__ == "__main__":
    ...
    for config_idx, config_path in enumerate(config_files):
        ...
        folder_name = f"sarsa_avg_{average_reward:.2f}_{timestamp}"
        ...
        with open(summary_file, "w") as f:
            f.write(f"SARSA Training Summary\n")
            f.write(f"{'='*60}\n\n")
            f.write(f"Algorithm: SARSA (On-Policy TD Control)\n")
    ...
    print(f"\n{'#'*80}")
    print(f"# SARSA - All Configurations Summary")
    print(f"{'#'*80}\n")
    ...
    print(f"\n{'='*80}")
    print(f"All SARSA runs complete! Results saved in results/ directory.")
    print(f"{'='*80}\n")
